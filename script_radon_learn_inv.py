...
train_params = {
    ...
    "acc_steps": [1],
    "train_transform": torchvision.transforms.Compose([Permute([1, 0])]),
    "val_transform": torchvision.transforms.Compose([Permute([1, 0])]),
...
for i in range(train_phases):
    train_params_cur = {}
    for key, value in train_params.items():
        #train_params_cur[key] = (value[i] if isinstance(value, (tuple, list)) else value)#下方嵌套if为修改
        if isinstance(value, (tuple, list)):
            if (len(value) == 1):
                train_params_cur[key] = (value[0])
            else:
                train_params_cur[key] = (value[i])
        else:
            train_params_cur[key] = value
